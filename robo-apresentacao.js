...
// Versão: 3.4 — MODO APRESENTAÇÃO
// =======================================================
// Este arquivo é uma cópia do reprocessar.js adaptada para
// demonstrações ao vivo. Diferenças em relação ao original:
//   • headless: false  → navegador visível
//   • slowMo: 1200ms   → todas as ações em câmera lenta
...
// ========================
async function pausaApresentacao(page, segundos, descricao) {
  log("INFO", `🎬 [APRESENTAÇÃO] ${descricao}`);
  log("INFO", `⏸️  Aguardando ${segundos}s para a audiência acompanhar...`);
...
(async () => {
  ...
  logDivisoria('🎬 MODO APRESENTAÇÃO — ROBO ASSIM');
  log('INFO', '🎬 Iniciando em modo de apresentação (navegador visível, velocidade reduzida)');

  const inicioTotal = Date.now();

  // ── Navegador visível + ações em câmera lenta ──────────────────────────────
  const browser = await chromium.launch({
    headless: false,
    slowMo: 1200      // 1,2 s entre cada ação do Playwright
  });

  // CSS, imagens e fontes carregam para o site aparecer com visual real
  ...
  // ── ETAPA 1: acesso ao site da ASSIM ───────────────────────────────────────
  log("INFO", "🎬 ETAPA 1 — Acessando o site da ASSIM Saúde");
  ...
  // ── ETAPA 2: login ─────────────────────────────────────────────────────────
  log("INFO", "🎬 ETAPA 2 — Realizando login no sistema");
  await page.selectOption('select', '52345');
  await page.fill('input[type="password"]', process.env.SENHA);

  await pausaApresentacao(page, 2, "Credenciais preenchidas — clicando em Entrar");
  ...
  await pausaApresentacao(page, 3, "Login realizado — painel do autorizador carregado");

  // ── Data ───────────────────────────────────────────────────────────────────
  let dia, mes, ano;
  if (process.env.TARGET_DATE) {
    [dia, mes, ano] = process.env.TARGET_DATE.split('-');
    log("INFO", `🗓️ Usando data definida via TARGET_DATE: ${dia}/${mes}/${ano}`);
  } else {
    const hoje = new Date();
  ...
  // ── ETAPA 3: extração do relatório normal ──────────────────────────────────
  log("INFO", `🎬 ETAPA 3 — Extraindo relatório Normal para ${dataHoje}`);
  ...
  await pausaApresentacao(page, 4, `Relatório Normal carregado — ${registrosNormal.length} registros encontrados`);

  // ── ETAPA 4: extração do relatório Prefeitura ──────────────────────────────
  log("INFO", `🎬 ETAPA 4 — Extraindo relatório Prefeitura para ${dataHoje}`);
  ...
  await pausaApresentacao(page, 4, `Relatório Prefeitura carregado — ${registrosPrefeitura.length} registros encontrados`);

  // ── ETAPA 5: envio ao Supabase ─────────────────────────────────────────────
  log("INFO", "🎬 ETAPA 5 — Enviando dados ao banco de dados Supabase");

  const registrosTodos = [...registrosNormal, ...registrosPrefeitura];
  log("INFO", `📋 Total: ${registrosNormal.length} Normal + ${registrosPrefeitura.length} Prefeitura = ${registrosTodos.length}`);

  const dadosBancoBruto = transformarParaSupabase(registrosTodos);
  const dadosBanco = removerDuplicadosPorGuia(dadosBancoBruto);

  log("INFO", `📦 Enviando ${dadosBanco.length} registros (sem duplicatas) em lotes`);
  await enviarEmLotes(dadosBanco);

  await pausaApresentacao(page, 3, `${dadosBanco.length} registros salvos no Supabase`);

  if (dadosBanco.length === 0) {
    log("INFO", "📭 Nenhum dado encontrado. Pulando geração de Excel e envio ao Órbita.");
    await browser.close();
    process.exit(0);
  }

  // ── ETAPA 6: gerar Excel ───────────────────────────────────────────────────
  log("INFO", "🎬 ETAPA 6 — Gerando arquivo Excel do relatório");
  ...
  await pausaApresentacao(page, 3, `Arquivo ${nomeArquivo} salvo em /relatorios`);

  // ── ETAPA 7: upload para o Google Drive ────────────────────────────────────
  log("INFO", "🎬 ETAPA 7 — Enviando relatório ao Google Drive");
  await enviarRelatorioDrive(caminhoArquivo, nomeArquivo);

  await pausaApresentacao(page, 3, "Relatório enviado ao Google Drive");

  // ── ETAPA 8: login no Órbita ───────────────────────────────────────────────
  ...
  log("INFO", "🎬 ETAPA 8 — Fazendo login no sistema Órbita");
  await loginOrbita(page, userOrbita, passOrbita);

  await pausaApresentacao(page, 3, "Login no Órbita realizado");

  // ── ETAPA 9: upload do Excel no Órbita ────────────────────────────────────
  log("INFO", `🎬 ETAPA 9 — Enviando Excel ao Órbita (data: ${dataHoje})`);
  await enviarExcelOrbita(page, caminhoArquivo, dataHoje);

  await pausaApresentacao(page, 4, "Upload concluído no Órbita — dados vinculados");

  // ── ETAPA 10: upload do log ────────────────────────────────────────────────
  log("INFO", "🎬 ETAPA 10 — Enviando arquivo de log ao Google Drive");
  ...
  await pausaApresentacao(page, 3, "Log enviado — execução concluída!");

  log("SUCCESS", `🏁 Execução finalizada em ${tempo(inicioTotal)}`);
  logDivisoria('🎬 FIM DA APRESENTAÇÃO');
